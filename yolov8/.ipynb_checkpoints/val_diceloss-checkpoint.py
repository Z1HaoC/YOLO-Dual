...
# 导入YOLOv8语义分割相关组件
from seg_diceloss_yolov8 import YOLOv8Seg, create_json_segment_dataloader
...
class SegmentationConfusionMatrix:
    ...
    def plot(self, save_dir, names):
        ...
        plt.xlabel('预测类别')
        plt.ylabel('真实类别')
        plt.title('YOLOv8 Segmentation Confusion Matrix (Dice)')
        plt.tight_layout()
...
def run(
        ...
        augment=False,
        verbose=False,
        project=ROOT / 'runs/val-yolov8-seg-dice',
        name='exp',
        exist_ok=False,
        half=True,
        model=None,
    ...
    # 打印评估结果
    LOGGER.info('\n' + '='*60)
    LOGGER.info(f"YOLOv8语义分割验证结果 (Dice) | 验证集总数: {seen} 张图像")
...
def parse_opt():
    ...
    parser.add_argument('--weights', type=str, 
                      default='runs/train-yolov8-seg-diceloss/weights/best.pt',
    ...
    parser.add_argument('--project', default=ROOT / 'runs/train-yolov8-seg-diceloss', 
                      help='Dice验证结果保存路径')
