...
# 从YOLOv9语义分割训练代码导入相关组件
from seg import YOLOv9Seg, create_json_segment_dataloader  # 核心修改：使用YOLOv9Seg模型
...
# CamVid颜色映射表（与其他版本完全一致，确保可视化颜色统一）
CAMVID_COLORS = [
...
class SegmentationConfusionMatrix:
    ...
    def plot(self, save_dir, names):
        ...
        plt.xlabel('预测类别')
        plt.ylabel('真实类别')
        plt.title('YOLOv9 Segmentation Confusion Matrix')  # 修改标题标识为YOLOv9
        plt.tight_layout()
        plt.savefig(save_dir / 'confusion_matrix.png')
        plt.close()


# -------------------------- 结果可视化函数（与其他版本完全一致，确保可视化效果统一） --------------------------
def visualize_results(img, pred_mask, true_mask, save_path, names):
    """可视化原图、真实掩码和预测掩码（格式与其他版本完全一致）"""
...
def run(
        ...
        augment=False,
        verbose=False,
        project=ROOT / 'runs/val-yolov9-seg',  # 修改保存路径为YOLOv9专用
        name='exp',
        exist_ok=False,
        half=True,
        model=None,
    ...
    if training:
        ...
        # 独立运行模式（加载YOLOv9Seg模型和数据集）
        device = select_device(device, batch_size=batch_size)
        # 创建结果保存目录（YOLOv9专用，避免与其他版本结果混淆）
        save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)
        (save_dir / 'visualizations').mkdir(parents=True, exist_ok=True)  # 可视化结果子目录
        
        # 加载数据集配置（与其他版本一致，JSON格式）
        data_dict = check_dataset(data)
        nc = 1 if single_cls else int(data_dict['nc'])  # 类别数（CamVid为12）
        
        # 加载YOLOv9Seg模型（核心修改：使用YOLOv9Seg模型和yolov9.yaml配置）
        model = YOLOv9Seg(cfg='yolov9.yaml', num_classes=nc).to(device)
        
        # 加载YOLOv9语义分割模型权重（适配YOLOv9训练结果）
        if weights:
        ...
        stride = model.stride  # 从YOLOv9Seg模型获取下采样倍数（[2,4,8,16,32]）
        imgsz = check_img_size(imgsz, s=stride)
        
        # 类别名称（与其他版本一致，CamVid-12类别）
        names = data_dict['names'] if not single_cls else ['class0']
        
        # 创建JSON格式数据加载器（与其他版本完全一致，确保数据输入统一）
        dataloader = create_json_segment_dataloader(
            img_dir=data_dict['val_img'],  # 验证集图像目录
            json_label_dir=data_dict['val_json'],  # 验证集JSON掩码目录（与其他版本共享）
            img_size=imgsz,
    ...
    # 开始验证（推理+评估）
    model.eval()
    pbar = tqdm(dataloader, desc=colorstr('val-yolov9-seg: '), bar_format=TQDM_BAR_FORMAT)  # 修改进度条标识
    for batch_i, (imgs, masks, paths) in enumerate(pbar):
        ...
        with dt[1]:  # 推理计时
            # YOLOv9Seg模型推理（输出shape: (batch, nc, H, W)）
        ...
        with dt[2]:  # 后处理计时
            ...
            for si in range(nb):
                ...
                if plots and batch_i < 3 and si < 3:
                    visualize_path = save_dir / 'visualizations' / f'{path.stem}_val_yolov9.png'  # 修改文件名标识
    ...
    # 打印评估结果（格式与其他版本完全一致，修改标题标识为YOLOv9）
    LOGGER.info('\n' + '='*60)
    LOGGER.info(f"YOLOv9语义分割验证结果 | 验证集总数: {seen} 张图像")
    ...
    # 返回评估指标（mIoU, 各类别IoU, 速度），与其他版本输出格式一致
    return (final_miou,), per_cls_iou, t


# -------------------------- 命令行参数解析（适配YOLOv9，修改默认路径） --------------------------
def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', type=str, default=ROOT / 'CamVid/data.yaml', 
                      help='数据集配置文件（与其他版本共享，JSON格式）')
    parser.add_argument('--weights', type=str, 
                      default='runs/train-yolov9-seg/weights/best.pt',  # 修改为YOLOv9最佳权重路径
                      help='YOLOv9语义分割模型权重路径')
    parser.add_argument('--batch-size', type=int, default=8, help='批量大小（与其他版本一致）')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, 
                      help='输入图像尺寸（与其他版本一致）')
    parser.add_argument('--device', default='', help='设备配置，例如 0 或 cpu（与其他版本一致）')
    parser.add_argument('--workers', type=int, default=4, help='数据加载线程数（与其他版本一致）')
    ...
    parser.add_argument('--verbose', action='store_true', help='详细输出每个类别的IoU（与其他版本一致）')
    parser.add_argument('--project', default=ROOT / 'runs/val-yolov9-seg', 
                      help='YOLOv9验证结果保存项目路径（修改为YOLOv9专用）')
    parser.add_argument('--name', default='exp', help='实验名称（默认exp）')
    parser.add_argument('--exist-ok', action='store_true', help='允许覆盖现有实验目录（与其他版本一致）')
