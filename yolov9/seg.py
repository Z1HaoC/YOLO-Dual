...
class Conv(nn.Module):
    """YOLOv9标准卷积模块：Conv2d -> BatchNorm2d -> SiLU"""
    def __init__(self, c1: int, c2: int, k: int = 1, s: int = 1, p: Optional[int] = None, 
        ...
        self.act = nn.SiLU() if act else nn.Identity()  # YOLOv9默认SiLU激活
...
class C3k2(nn.Module):
    """YOLOv9的C3k2模块 (CSPNetv2结构) - 替代YOLOv8的C2f模块"""
...
class GAM(nn.Module):
    """全局聚合模块 (Global Aggregation Module) - YOLOv9核心改进"""
    def __init__(self, c, k=7):
        super().__init__()
        self.conv1 = Conv(c, c, k, 1, k//2, c)  # 深度可分离卷积
        self.conv2 = Conv(c, c, 1)  # 1x1卷积调整通道
        self.act = nn.Sigmoid()

    def forward(self, x):
        return x * self.act(self.conv2(self.conv1(x))) + x  # 残差连接
...
class YOLOv9Seg(nn.Module):
    """完整的YOLOv9语义分割模型（编码器：YOLOv9 backbone，解码器：统一分割头）"""
    def __init__(self, cfg: Dict, num_classes: Optional[int] = None):
        ...
        self.stride = torch.tensor([2, 4, 8, 16, 32])  # YOLOv9各阶段下采样倍数
    ...
    def _build_backbone(self, cfg):
        """构建YOLOv9 backbone（使用C3k2和GAM模块）"""
        ...
        for layer_idx, layer in enumerate(cfg):
            ...
            elif module == 'C3k2':  # 使用C3k2替代C2f
                c2 = args[0]
                layer_module = C3k2(c1, *args)
            elif module == 'GAM':  # 全局聚合模块
                c2 = c1  # GAM不改变通道数
    ...
    def _build_head(self, cfg, backbone_out_chs):
            ...
            for i, layer in enumerate(cfg):
                ...
                if module == 'Conv':
                    ...
                    raise NotImplementedError(f"YOLOv9分割头未知模块: {module}")

                head.append(layer_module)
                all_out_chs.append(c2)
                LOGGER.debug(f"YOLOv9 Head层 {i}: {module} → 输入{c1}→输出{c2}通道")
...
def train(hyp: Dict, opt: argparse.Namespace, device: torch.device, callbacks: Callbacks) -> Tuple[float, ...]:
    ...
    model = YOLOv9Seg(opt.cfg, num_classes=num_classes).to(device)  # 使用YOLOv9Seg模型
    ...
    # 训练循环
    LOGGER.info(colorstr('开始训练（YOLOv9 Backbone语义分割版）!'))
    t0 = time.time()
    global_step = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='', help='初始权重路径（YOLOv9预训练权重或自定义权重）')
    parser.add_argument('--cfg', type=str, default='yolov9.yaml', help='模型配置文件（YOLOv9语义分割版）')
    ...
    parser.add_argument('--save-dir', type=str, default='runs/train-yolov9-seg', help='结果保存目录（YOLOv9专用）')
