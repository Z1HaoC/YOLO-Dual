...
if str(PROJECT_ROOT) not in sys.path:
    ...
    print(f"已添加项目根目录到环境变量: {PROJECT_ROOT}")
...
# 工具导入
import val_diceloss as validate  # 验证与评估（复用之前的val.py）
...
# CamVid颜色映射表(RGB)用于可视化（与VGG16版本完全一致）
CAMVID_COLORS = [
...
# -------------------------- YOLOv5 Backbone 核心模块（基于yolov5.yaml定义） --------------------------
...
class Conv(nn.Module):
    """YOLOv5标准卷积模块：Conv2d -> BatchNorm2d -> SiLU（区别于VGG16的ReLU）"""
    def __init__(self, c1: int, c2: int, k: int = 1, s: int = 1, p: Optional[int] = None, 
        ...
        self.act = nn.SiLU() if act else nn.Identity()  # YOLOv5默认SiLU激活
...
class C3(nn.Module):
    """YOLOv5 C3模块（无DCN版本，与yolov5.yaml一致）"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # 隐藏通道数（按比例计算）
...
class C3_DCN(nn.Module):
    """YOLOv5 C3-DCN模块（带可变形卷积，补充offset生成逻辑）"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # 隐藏通道数
        self.cv1 = Conv(c1, c_, 1, 1)  # 分支1：降维卷积
        self.cv2 = Conv(c1, c_, 1, 1)  # 分支2：直接降维（无DCN）
        self.cv3 = Conv(2 * c_, c2, 1) # 最终拼接后升维
        self.add = shortcut and c1 == c2  # 残差连接开关

        # 构建DCN子模块（每个子模块包含：Conv+offset生成+DeformConv2d+BN+SiLU）
        dcn_blocks = []
        try:
        ...
        for _ in range(n):
            # 1. 先对输入特征做一次普通卷积（调整通道+提取基础特征）
            conv = Conv(c_, c_, 3, 1, g=g, act=False)  # 无激活，后续统一用SiLU
            # 2. 偏移量生成层：输出通道=2*k*k（k=3，对应x/y方向各9个偏移）
            offset_conv = Conv(c_, 2*3*3, 3, 1, g=g, act=True)  # 输出18通道（2*3*3），用SiLU激活
            # 3. 可变形卷积层（无偏置，后续接BN）
            ...
            # 组合单个DCN块：Conv → 生成offset → DCN卷积 → BN+SiLU
            dcn_blocks.append(nn.Sequential(conv, offset_conv, dcn_conv, bn_silu))
        
        self.m = nn.Sequential(*dcn_blocks)  # n个DCN块串联
    ...
    def forward(self, x):
        ...
        for block in self.m:
            # 每个DCN块的前向逻辑：
            # block[0] = 普通Conv；block[1] = offset生成层；block[2] = DeformConv2d；block[3] = BN+SiLU
            x1 = block[0](x1)          # 普通卷积预处理
            offset = block[1](x1)      # 生成偏移量（N, 18, H, W）
            x1 = block[2](x1, offset)  # 传入offset执行可变形卷积
...
# -------------------------- YOLOv5Seg 语义分割模型（基于yolov5.yaml构建） --------------------------
class YOLOv5Seg(nn.Module):
    """完整的YOLOv5语义分割模型（编码器：YOLOv5 backbone，解码器：统一分割头）"""
    def __init__(self, cfg: Dict, num_classes: Optional[int] = None):
        ...
        self.stride = torch.tensor([2, 4, 8, 16, 32])  # YOLOv5各阶段下采样倍数
    ...
    def _build_head(self, cfg, backbone_out_chs):
            ...
            for i, layer in enumerate(cfg):
                ...
                if module == 'Conv':
                    ...
                    raise NotImplementedError(f"YOLOv5分割头未知模块: {module}")

                head.append(layer_module)
                all_out_chs.append(c2)
                LOGGER.debug(f"YOLOv5 Head层 {i}: {module} → 输入{c1}→输出{c2}通道")
    ...
    def _initialize_weights(self) -> None:
        """权重初始化（兼容PyTorch 1.10+，无gain参数，彻底解决API冲突）"""
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                # 关键修改：仅使用nonlinearity='leaky_relu'，不使用gain参数
                # 理由：PyTorch 1.10+对leaky_relu的支持稳定，且与SiLU初始化效果相近
...
def train(hyp: Dict, opt: argparse.Namespace, device: torch.device, callbacks: Callbacks) -> Tuple[float, ...]:
    ...
    if RANK in {-1, 0}:
        ...
        # 关键修复：注册回调前先过滤None值和非可调用对象
        for k in dir(loggers):
            if k.startswith('on_') and not k.startswith('__'):  # 仅处理回调方法，跳过魔法方法
                callback = getattr(loggers, k, None)  # 不存在的属性返回None
                # 先判断是否为None，再判断是否可调用
                ...
                # else:
                #     LOGGER.debug(f"跳过无效回调 {k}: {'None' if callback is None else '不可调用'}")
        
        data_dict = loggers.remote_dataset

    # 数据集配置（与VGG16版本一致，JSON格式）
    ...
    model = YOLOv5Seg(opt.cfg, num_classes=num_classes).to(device)  # 核心修改：使用YOLOv5Seg模型
    
    # 加载预训练权重（与VGG16版本逻辑一致）
    if pretrained:
    ...
    # 根据命令行参数控制AMP启用状态，默认禁用
    amp_enabled = opt.amp and cuda  # 只有显式指定--amp且使用GPU时才启用AMP
    scaler = None
    if amp_enabled:
        try:
            # 针对PyTorch 1.10.x使用torch.cuda.amp
    ...
    # 训练循环（与VGG16版本一致）
    LOGGER.info(colorstr('开始训练（YOLOv5 Backbone语义分割版）!'))
    t0 = time.time()
    global_step = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='weight.yaml', help='初始权重路径（YOLOv5预训练权重或自定义权重）')
    parser.add_argument('--cfg', type=str, default='yolov5.yaml', help='模型配置文件（YOLOv5语义分割版）')
    parser.add_argument('--data', type=str, default='/root/BestYOLO/CamVid/data.yaml', help='数据集配置文件（JSON版本）')
    parser.add_argument('--epochs', type=int, default=300, help='训练轮数')
    parser.add_argument('--batch-size', type=int, default=4, help='批量大小（与VGG16版本一致，避免显存差异）')
    ...
    parser.add_argument('--patience', type=int, default=150, help='早停 patience（与VGG16版本一致）')
    ...
    parser.add_argument('--save-dir', type=str, default='runs/train-yolov5-seg', help='结果保存目录（YOLOv5专用）')
    ...
    parser.add_argument('--hyp', type=str, default='/root/BestYOLO/data/hyps/hyp.scratch-seg.yaml', help='超参数配置文件（与VGG16版本一致）')
