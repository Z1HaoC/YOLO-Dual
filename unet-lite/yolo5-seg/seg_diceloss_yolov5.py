...
# 项目根目录追加（确保工具类导入正常）
PROJECT_ROOT = Path("/root/BestYOLO")
if str(PROJECT_ROOT) not in sys.path:
    sys.path.append(str(PROJECT_ROOT))
    LOGGER.info(f"已添加项目根目录到环境变量: {PROJECT_ROOT}")
# 分布式训练配置
LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))
RANK = int(os.getenv('RANK', -1))
WORLD_SIZE = int(os.getenv('WORLD_SIZE', 1))

# CamVid颜色映射表(RGB)与类别定义
CAMVID_COLORS = [
...
class JSONSegmentDataset(Dataset):
    def __init__(self, img_dir, json_label_dir, img_size=640, augment=False, hyp=None):
        ...
        if self.num_classes != NUM_CLASSES:
            LOGGER.warning(f"JSON文件类别数({self.num_classes})与默认({NUM_CLASSES})不匹配，将使用JSON定义类别")
    ...
    def _resize_and_pad(self, img, mask):
        ...
        new_mask = Image.new('L', (self.img_size, self.img_size), 0)  # 0为默认背景类
...
def create_json_segment_dataloader(img_dir, json_label_dir, img_size=640, batch_size=16, 
    ...
    if batch_size > len(dataset):
        ...
        LOGGER.warning(f"批次大小调整为 {batch_size}（数据集仅含{len(dataset)}样本）")
...
class Conv(nn.Module):
    """YOLOv5标准卷积模块：Conv2d -> BatchNorm2d -> SiLU"""
...
class C3(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.add = shortcut and c1 == c2  # 残差连接开关
...
class C3_DCN(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        try:
            ...
            raise ImportError("使用C3-DCN需安装torchvision>=0.11.0，执行：pip install torchvision>=0.11.0")
        
        for _ in range(n):
            conv = Conv(c_, c_, 3, 1, g=g, act=False)
            offset_conv = Conv(c_, 2*3*3, 3, 1, g=g, act=True)  # 输出18通道（x/y各9个偏移）
            dcn_conv = DeformConv2d(c_, c_, kernel_size=3, padding=1, groups=g, bias=False)
            bn_silu = nn.Sequential(nn.BatchNorm2d(c_), nn.SiLU(inplace=True))
            dcn_blocks.append(nn.Sequential(conv, offset_conv, dcn_conv, bn_silu))
        
        self.m = nn.Sequential(*dcn_blocks)

    def forward(self, x):
        x1 = self.cv1(x)
        for block in self.m:
            x1 = block[0](x1)          # 普通卷积预处理
            offset = block[1](x1)      # 生成偏移量
            x1 = block[2](x1, offset)  # 可变形卷积
            x1 = block[3](x1)          # BN+SiLU激活
        x2 = self.cv2(x)
        return self.cv3(torch.cat([x1, x2], dim=1)) + (x if self.add else 0)


class SPPF(nn.Module):
    """YOLOv5空间金字塔池化-快速版（减少计算量）"""
...
class YOLOv5Seg(nn.Module):
    ...
    def __init__(self, cfg: Dict, num_classes: Optional[int] = None):
        ...
        self.stride = torch.tensor([2, 4, 8, 16, 32])  # 各阶段下采样倍数
    ...
    def _build_head(self, cfg, backbone_out_chs):
        ...
        for i, layer in enumerate(cfg):
            ...
            if isinstance(from_, list):
                c1 = sum([all_out_chs[f] for f in from_])
            else:
            ...
            if module == 'Conv':
                c2 = args[0]
                layer_module = Conv(c1, *args)
            elif module == 'Upsample' or module == 'nn.Upsample':
                c2 = c1  # 上采样不改变通道数
                # 解析参数（修复类型错误）
                size_arg, scale_factor_arg, mode = args[0] if len(args)>=1 else None, args[1] if len(args)>=2 else 2.0, args[2] if len(args)>=3 else 'nearest'
                
                # 处理size/scale_factor
                size, scale_factor = None, None
                if size_arg is not None:
                    try:
                        size = tuple(map(int, size_arg[-2:])) if isinstance(size_arg, (list, tuple)) else (int(size_arg), int(size_arg))
                    except:
                        size = None
                if scale_factor is None and size is None:
                    scale_factor = float(scale_factor_arg) if isinstance(scale_factor_arg, (int, float)) else 2.0

                # 创建Upsample层
                upsample_kwargs = {'mode': mode, 'align_corners': False if mode in ['bilinear', 'bicubic'] else None}
                if size is not None:
                    upsample_kwargs['size'] = size
                else:
                    upsample_kwargs['scale_factor'] = scale_factor
                layer_module = nn.Upsample(**upsample_kwargs)
            elif module == 'Concat':
                c2 = c1
                ...
                # 添加对nn.Softmax的支持
                c2 = c1  # Softmax不改变通道数
                dim = args[0] if args else 1  # 默认在通道维度上应用
            ...
            LOGGER.debug(f"分割头层 {i}: {module} → 输入{c1}→输出{c2}通道")
    ...
    def _forward_once(self, x: torch.Tensor, profile: bool = False) -> torch.Tensor:
        ...
        for layer, layer_cfg in zip(self.head, self.yaml['head']):
            ...
            # 执行计算
            x = layer(inputs) if isinstance(layer, Concat) else layer(inputs)
            x_outs.append(x)
    ...
    def _log_model_info(self) -> None:
        ...
        LOGGER.info(f"计算量: {flops/1e9:.2f} GFLOPs (输入尺寸: {self.img_size})")
...
class SegmentationLoss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, target: torch.Tensor) -> Tuple[torch.Tensor, List[float]]:
        ...
        if pred.size(0) != target.size(0):
            raise ValueError(f"批次大小不匹配: 输出{pred.size(0)} vs 标签{target.size(0)}")
...
def seg_labels_to_class_weights(json_files, num_classes):
    ...
    weights = total / (num_classes * (class_counts + 1e-8))  # 避免除以零
    return torch.from_numpy(weights).float()


def parse_class_weights(weight_str, num_classes):
    """解析命令行输入的类别权重（支持文件/字符串）"""
    if not weight_str:
        return None
        ...
        try:
            ...
            if isinstance(weights, dict):
                ...
                if len(weights) != num_classes:
                    raise ValueError(f"权重数量({len(weights)})与类别数({num_classes})不匹配")
                weight_list = weights
            else:
...
def mask_to_rgb(mask, color_map=CAMVID_COLORS):
    """掩码转RGB可视化"""
    h, w = mask.shape
...
def visualize_prediction_difference(gt_mask, pred_mask, color_map=CAMVID_COLORS):
    """可视化预测与GT的差异（洋红色标记错误）"""
    h, w = gt_mask.shape
...
def train(hyp: Dict, opt: argparse.Namespace, device: torch.device, callbacks: Callbacks) -> Tuple[float, ...]:
    ...
    if not verify_json_masks(train_img_dir, train_json_dir):
        ...
        if train_success == 0 and train_total > 0:
            LOGGER.error("训练集掩码转换失败，终止训练")
            return (0.0,)
        if not verify_json_masks(train_img_dir, train_json_dir):
            LOGGER.error("训练集JSON验证仍失败，终止训练")
            return (0.0,)
    # 验证集
    if not verify_json_masks(val_img_dir, val_json_dir):
        ...
        if val_success == 0 and val_total > 0:
            LOGGER.error("验证集掩码转换失败，终止训练")
            return (0.0,)
        if not verify_json_masks(val_img_dir, val_json_dir):
            LOGGER.error("验证集JSON验证仍失败，终止训练")
            return (0.0,)
    ...
    model = YOLOv5Seg(opt.cfg, num_classes=num_classes).to(device)
    
    # 加载预训练权重（兼容YOLOv5权重格式）
    if pretrained:
    ...
    if opt.class_weights:
        ...
        LOGGER.info(f"使用数据驱动类别权重: {class_weights.tolist()}")
    ...
    if amp_enabled:
        try:
            ...
            LOGGER.warning("PyTorch版本不支持AMP，禁用AMP")
            amp_enabled = False
            scaler = None
    else:
        LOGGER.info("禁用自动混合精度训练(AMP)")

    # 早停配置
    stopper = EarlyStopping(patience=opt.patience)

    # 训练循环
    LOGGER.info(colorstr('开始训练（YOLOv5 Backbone语义分割版 - Dice损失）!'))
    t0 = time.time()
    global_step = 0
    ...
    for epoch in range(start_epoch, epochs):
            ...
            if RANK in {-1, 0}:
                ...
                if global_step % 100 == 0 and global_step != 0:
                    ...
                    for s in range(num_samples):
                        ...
                        # 转换为RGB可视化
                        target_rgb = mask_to_rgb(target_np)
                        pred_rgb = mask_to_rgb(pred_np)
                        diff_rgb = visualize_prediction_difference(target_np, pred_np)
        ...
        if RANK in {-1, 0}:
            ...
            # 模型保存
            fi = fitness(np.array(results).reshape(1, -1))  # 计算适应度（mIoU为主）
            if fi > best_fitness:
                best_fitness = fi
            # 保存条件：不禁用保存 或 最后一轮且不进化超参数
            ...
            if stopper(epoch=epoch, fitness=fi):
                LOGGER.info(f"早停触发: 第{epoch}轮，当前最佳mIoU: {best_fitness:.4f}")
                break
    ...
    if RANK in {-1, 0}:
        ...
        LOGGER.info(f'\n训练完成 ({train_time:.2f} 小时)')
        LOGGER.info(f"最佳模型保存至: {best}")
        strip_optimizer(best)  # 移除优化器状态以减小模型体积
...
if __name__ == '__main__':
    ...
    # 模型配置
    parser.add_argument('--weights', type=str, default='', help='初始权重路径（YOLOv5预训练权重/自定义权重）')
    parser.add_argument('--cfg', type=str, default='yolov5_seg.yaml', help='模型配置文件（YOLOv5语义分割版）')
    parser.add_argument('--data', type=str, default='/root/BestYOLO/CamVid/data.yaml', help='数据集配置文件（JSON版本）')
    # 训练参数
    parser.add_argument('--epochs', type=int, default=300, help='训练轮数')
    parser.add_argument('--batch-size', type=int, default=4, help='批量大小（根据显存调整）')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, help='输入图像尺寸')
    parser.add_argument('--device', default='', help='设备配置: 0 或 0,1,2,3 或 cpu')
    parser.add_argument('--workers', type=int, default=8, help='数据加载线程数')
    parser.add_argument('--freeze', nargs='+', type=int, default=[0], help='冻结Backbone层数（如 [10] 冻结前10层）')
    parser.add_argument('--patience', type=int, default=150, help='早停耐心值（无提升轮数）')
    # 训练策略
    parser.add_argument('--single-cls', action='store_true', help='单类别训练')
    parser.add_argument('--sync-bn', action='store_true', help='分布式训练使用同步BN')
    parser.add_argument('--cos-lr', action='store_true', help='使用余弦学习率调度')
    parser.add_argument('--resume', nargs='?', const=True, default=False, help='恢复训练（默认从last.pt恢复）')
    parser.add_argument('--amp', action='store_true', default=False, help='启用自动混合精度训练（默认禁用）')
    # 类别权重
    parser.add_argument('--class-weights', type=str, default='weight.yaml', help='自定义类别权重（文件路径或逗号分隔字符串）')
    # 输出配置
    parser.add_argument('--save-dir', type=str, default='runs/train-yolov5-seg-diceloss', help='结果保存目录')
    parser.add_argument('--noval', action='store_true', help='禁用验证（仅训练）')
    parser.add_argument('--nosave', action='store_true', help='不保存模型（仅测试）')
    parser.add_argument('--noplots', action='store_true', help='不生成可视化图表')
    # 数据加载
    parser.add_argument('--cache', type=str, nargs='?', const='ram', help='缓存图像到内存/磁盘（ram/disk）')
    parser.add_argument('--rect', action='store_true', help='使用矩形训练（减少Padding）')
    parser.add_argument('--image-weights', action='store_true', help='使用图像权重采样（平衡数据分布）')
    parser.add_argument('--quad', action='store_true', help='使用四分之一采样（加速训练）')
    # 超参数
    parser.add_argument('--evolve', action='store_true', help='进化超参数（仅测试）')
    ...
    # 其他
    parser.add_argument('--optimizer', type=str, default='SGD', help='优化器类型（SGD/Adam）')
    parser.add_argument('--label-smoothing', type=float, default=0.0, help='标签平滑因子（0-1）')
