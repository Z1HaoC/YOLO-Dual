...
class Conv(nn.Module):
    """VGG标准卷积模块：Conv2d -> BatchNorm2d -> ReLU（VGG专用激活）"""
    def __init__(self, c1: int, c2: int, k: int = 1, s: int = 1, p: Optional[int] = None, 
        ...
        self.act = nn.ReLU(inplace=True) if act else nn.Identity()  # VGG使用ReLU激活
...
class VGGBlock(nn.Module):
    """VGG网络基本块：多个Conv层 +  MaxPool2d（可选）"""
    def __init__(self, in_channels: int, out_channels: int, num_convs: int, pool: bool = True):
        super().__init__()
        layers = []
        for _ in range(num_convs):
            layers.append(Conv(in_channels, out_channels, k=3, s=1, act=True))
            in_channels = out_channels
        if pool:
            layers.append(nn.MaxPool2d(kernel_size=2, stride=2))  # VGG默认2x2池化（步长2）
...
class VGG16Seg(nn.Module):
    """VGG16语义分割模型（编码器：VGG16 backbone，解码器：与YOLOv8统一的分割头）"""
    def __init__(self, cfg: Dict, num_classes: Optional[int] = None):
        ...
        self.stride = torch.tensor([2, 4, 8, 16, 16])  # VGG16各阶段下采样倍数（最后一层不缩减）
        
        # 构建主干网络和分割头（仅替换Backbone为VGG16）
        ...
        # 初始化权重与日志（与YOLOv8保持一致）
        self._initialize_weights()
        self._log_model_info()

# 修改YOLOv8Seg类中的_build_backbone方法，统一Upsample参数解析逻辑

    def _build_backbone(self, cfg):
        """构建YOLOv8 backbone（使用新配置）"""
        ...
        for layer_idx, layer in enumerate(cfg):
            ...
            # 应用通道缩放（与其他版本保持一致）
            if module in ['Conv', 'C2f', 'C2f_DCN', 'SPPF']:
                args[0] = int(args[0] * self.width_multiple)
                if args[0] % 2 != 0:
                    args[0] += 1  # 确保通道数为偶数
            
            # 应用深度缩放
            if module in ['C2f', 'C2f_DCN']:
                num_ = max(round(num_ * self.depth_multiple), 1)
                args[1] = num_  # 更新shortcut参数位置

            # 构建模块
            if module == 'Conv':
                c2 = args[0]
                layer_module = Conv(c1, *args)
            elif module == 'C2f':  # 使用C2f模块
                c2 = args[0]
                ...
                c2 = c1  # 上采样不改变通道数
                # 解析参数 - 确保size和scale_factor互斥（与head中逻辑一致）
                size = None
                scale_factor = None
                mode = 'nearest'  # 默认模式
                ...
                if len(args) >= 1:
                    ...
                    if isinstance(first_arg, dict):
                        # 支持字典形式配置
                        size = first_arg.get('size', None)
                        scale_factor = first_arg.get('scale_factor', None)
                        mode = first_arg.get('mode', mode)
                    else:
                        # positional参数形式
                        if len(args) >= 3:
                            ...
                            size_arg = None
                            scale_factor_arg = args[0] if len(args) > 0 else 2.0
                            mode = args[1] if len(args) > 1 else mode

                        # 处理size参数
                        if size_arg is not None:
                            if isinstance(size_arg, (list, tuple)):
                                size = tuple(int(x) for x in size_arg[-2:])
                        ...
                        if scale_factor_arg is not None:
                            if isinstance(scale_factor_arg, (list, tuple)):
                                scale_factor = tuple(float(x) for x in scale_factor_arg[-2:])
                            elif isinstance(scale_factor_arg, (int, float)):
                                scale_factor = float(scale_factor_arg)

                # 强制互斥性：确保只有一个参数有效
                # 优先使用size，如果设置了size则清除scale_factor
                if size is not None:
                    scale_factor = None
                else:
                    # 如果没有设置size，则确保scale_factor有默认值
                    if scale_factor is None:
                        scale_factor = 2.0  # 默认缩放2倍
                    # 清除size以确保互斥
                    size = None
                ...
                assert not (size is not None and scale_factor is not None), \
                    "Upsample层只能设置size或scale_factor中的一个参数"
    ...
    def _build_head(self, cfg, backbone_out_chs):
        ...
        for i, layer in enumerate(cfg):
            ...
            if module == 'Conv':
                ...
                c2 = c1
                # 解析上采样参数（与YOLOv8参数处理逻辑一致）
                if len(args) >= 3:
                    size_arg, scale_factor_arg, mode = args[0], args[1], args[2]
                else:
                    size_arg = None
                    scale_factor_arg = 2.0
                    mode = 'nearest'
                ...
                raise NotImplementedError(f"VGG16分割头未知模块: {module}")

            head.append(layer_module)
            all_out_chs.append(c2)
            LOGGER.debug(f"VGG16 Head层 {i}: {module} → 输入{c1}→输出{c2}通道")
    ...
    def _initialize_weights(self) -> None:
        """VGG风格权重初始化（适配Backbone特性）"""
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')  # ReLU适配
    ...
    def _log_model_info(self) -> None:
        ...
        LOGGER.info(f"VGG16Seg模型信息: {n_params:,} 总参数, {n_trainable:,} 可训练参数")
...
class SegmentationLoss(nn.Module):
    """语义分割损失（交叉熵+Dice，与YOLOv8完全一致，支持类别权重）"""
...
def train(hyp: Dict, opt: argparse.Namespace, device: torch.device, callbacks: Callbacks) -> Tuple[float, ...]:
    ...
    model = VGG16Seg(opt.cfg, num_classes=num_classes).to(device)  # 使用VGG16Seg模型
    ...
    # 训练循环
    LOGGER.info(colorstr('开始训练（VGG16 Backbone语义分割版 - Dice损失）!'))
    t0 = time.time()
    global_step = 0
...
# -------------------------- 命令行参数与主函数（与YOLOv8完全一致） --------------------------
if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--weights', type=str, default='', help='初始权重路径（VGG16预训练权重或自定义权重）')
    parser.add_argument('--cfg', type=str, default='vgg16.yaml', help='模型配置文件（VGG16语义分割版）')
    ...
    parser.add_argument('--save-dir', type=str, default='runs/train-vgg16-seg-diceloss', help='结果保存目录（Dice损失专用）')
