...
# 从seg.py导入VGG相关组件（替换ResNet34组件）
from seg_diceloss_vgg16 import VGG16Seg, create_json_segment_dataloader  # 修改：使用VGG16Seg和JSON数据加载器
...
def run(
        ...
        augment=False,
        verbose=False,
        project=ROOT / 'runs/val-vgg16',  # 修改：VGG专用保存路径
        name='exp',
        exist_ok=False,
        half=True,
        model=None,
    ...
    if training:
        ...
        model = VGG16Seg(cfg='vgg16.yaml', num_classes=nc).to(device)  # 修改：使用VGG16Seg和对应配置文件
        ...
        # 创建数据加载器 - 使用JSON版本的数据加载器
        dataloader = create_json_segment_dataloader(  # 修改：使用JSON数据加载器
            img_dir=data_dict['val_img'],
            json_label_dir=data_dict['val_json'],  # 修改：使用JSON标签目录
            img_size=imgsz,
...
def parse_opt():
    ...
    parser.add_argument('--weights', type=str, default='runs/train-vgg16-augmented/weights/best.pt', 
                      help='模型路径（VGG16最佳权重）')  # 修改：VGG模型权重路径
    ...
    parser.add_argument('--project', default=ROOT / 'runs/val-vgg16',  # 修改：VGG专用项目路径
                      help='保存项目路径')
