...
from seg_json import ResNet34Seg, create_json_segment_dataloader  # 修改：使用JSON数据加载器
...
def run(
    ...
    if training:
        ...
        # 加载模型 - 使用从seg导入的ResNet34Seg
        data_dict = check_dataset(data)
        nc = 1 if single_cls else int(data_dict['nc'])
        model = ResNet34Seg(cfg='resnet34.yaml', num_classes=nc).to(device)  # 修改：使用ResNet34Seg和对应配置文件
        ...
        dataloader = create_json_segment_dataloader(
            ...
            json_label_dir=data_dict.get('val_json', os.path.join(data_dict['val_img'], 'json')),  # 使用JSON掩码目录
            img_size=imgsz,
