...
class ResNet18(nn.Module):
    """ResNet18主干网络（与YOLOv8/VGG16的特征提取接口一致）"""
    def __init__(self):
        super().__init__()
        ...
        # 四个主要阶段
        self.layer1 = self._make_layer(64, 2, stride=1)    # ResNet18使用2个块
        self.layer2 = self._make_layer(128, 2, stride=2)   # ResNet18使用2个块
        self.layer3 = self._make_layer(256, 2, stride=2)   # ResNet18使用2个块
        self.layer4 = self._make_layer(512, 2, stride=2)   # ResNet18使用2个块
    ...
    def forward(self, x: torch.Tensor) -> List[torch.Tensor]:
        ...
        # 检查输入通道是否为3
        if x.size(1) != 3:
            raise ValueError(f"ResNet18期望输入3通道图像，实际输入{ x.size(1) }通道")
...
class ResNet18Seg(nn.Module):
    """完整的ResNet18语义分割模型（与YOLOv8/VGG16模型接口一致）"""
...
def train(hyp: Dict, opt: argparse.Namespace, device: torch.device, callbacks: Callbacks) -> Tuple[float, ...]:
    ...
    model = ResNet18Seg(opt.cfg, num_classes=num_classes).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='resnet18.yaml', help='模型配置文件')
    ...
    parser.add_argument('--save-dir', type=str, default='runs/train-resnet18', help='结果保存目录')
