...
# 导入YOLOv9语义分割相关组件
from seg_diceloss_yolov9 import YOLOv9Seg, create_json_segment_dataloader, seg_labels_to_class_weights, parse_class_weights
...
class SegmentationConfusionMatrix:
    ...
    def plot(self, save_dir, names):
        ...
        plt.xlabel('预测类别')
        plt.ylabel('真实类别')
        plt.title('YOLOv9 Segmentation Confusion Matrix (Dice)')
        plt.tight_layout()
...
def visualize_results(img, pred_mask, true_mask, save_path, names):
    ...
    # 计算差异图像
    diff_colored = visualize_prediction_difference(true_mask, pred_mask, CAMVID_COLORS)
    
    pred_overlay = cv2.addWeighted(img_np, 0.5, pred_colored, 0.5, 0)
    true_overlay = cv2.addWeighted(img_np, 0.5, true_colored, 0.5, 0)
    
    h, w = img_np.shape[:2]
    title = np.ones((50, w*4, 3), dtype=np.uint8) * 255
    ...
    cv2.putText(title, '差异区域', (3*w + w//3, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2)
    
    combined = np.vstack((title, np.hstack((img_np, true_overlay, pred_overlay, diff_colored))))
...
def run(
        ...
        augment=False,
        verbose=False,
        project=ROOT / 'runs/val-yolov9-seg-dice',
        name='exp',
        exist_ok=False,
        half=True,
        model=None,
    ...
    if training:
        ...
        if class_weights:
            # 解析用户提供的类别权重
            class_weights = parse_class_weights(class_weights, nc)
            LOGGER.info(f"使用自定义类别权重: {class_weights.tolist()}")
        else:
            # 尝试从数据集中计算权重
            try:
                json_files = list(Path(data_dict['val_json']).glob("*.json"))
                if json_files:
                    class_weights = seg_labels_to_class_weights(json_files, nc)
                    LOGGER.info(f"使用数据驱动的类别权重: {class_weights.tolist()}")
            except:
                class_weights = None
                LOGGER.warning("无法计算数据驱动的类别权重，将使用均匀权重")
        
        # 初始化Dice损失计算器
        dice_loss_fn = DiceLoss(num_classes=nc, class_weights=class_weights)
        
        # 加载YOLOv9Seg模型
        model = YOLOv9Seg(cfg='yolov9_seg.yaml', num_classes=nc).to(device)
    ...
    for batch_i, (imgs, masks, paths) in enumerate(pbar):
        ...
        with dt[1]:
            ...
            total_dice_loss += batch_loss.item() * nb  # 累积批次损失
    ...
    # 打印评估结果
    LOGGER.info('\n' + '='*60)
    LOGGER.info(f"YOLOv9语义分割验证结果 (Dice) | 验证集总数: {seen} 张图像")
    LOGGER.info(f"整体mIoU: {final_miou:.4f}")
    LOGGER.info(f"平均Dice损失: {avg_dice_loss:.4f}")  # 重点展示Dice损失
    LOGGER.info('-'*60)
...
def parse_opt():
    ...
    parser.add_argument('--weights', type=str, 
                      default='runs/train-yolov9-seg-diceloss/weights/best.pt',
                      help='YOLOv9语义分割模型权重路径')
    parser.add_argument('--batch-size', type=int, default=4, help='批量大小')  # 与训练脚本保持一致
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=640, 
                      help='输入图像尺寸')
    parser.add_argument('--device', default='', help='设备配置，例如 0 或 cpu')
    parser.add_argument('--workers', type=int, default=8, help='数据加载线程数')  # 与训练脚本保持一致
    ...
    parser.add_argument('--project', default=ROOT / 'runs/train-yolov9-seg-diceloss', 
                      help='Dice验证结果保存路径')
    ...
    parser.add_argument('--hyp', type=str, default='data/hyps/hyp.scratch-seg.yaml', help='超参数配置文件')
