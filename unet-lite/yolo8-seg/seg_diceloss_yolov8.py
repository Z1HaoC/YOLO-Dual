...
# 工具导入
import val_diceloss as validate  # 验证与评估（使用Dice损失版本的验证脚本）
...
class C3(nn.Module):
    """C3模块：基于CSP结构的改进版，用于YOLOv5及相关模型"""
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__()
        c_ = int(c2 * e)  # 隐藏通道数
...
class YOLOv8Seg(nn.Module):
    ...
    def _build_backbone(self, cfg):
        ...
        for layer_idx, layer in enumerate(cfg):
            ...
            if module == 'Conv':
            ...
            elif module == 'Upsample':  # 完善Upsample模块实现
                c2 = c1  # 上采样不改变通道数
                # 解析参数
                if len(args) >= 3:
                ...
                if size_arg is not None:
                    try:
                        ...
                        size = None  # 解析失败则不设置
                elif scale_factor_arg is not None:
                    try:
                        ...
                        scale_factor = None  # 解析失败则不设置

                # 创建Upsample层（确保只传递非None的参数）
                upsample_kwargs = {
                    'mode': mode,
                ...
                if size is not None:
                    ...
                    # 默认使用scale_factor=2.0
                    upsample_kwargs['scale_factor'] = 2.0
    ...
    def _build_head(self, cfg, backbone_out_chs):
        """构建统一分割头"""
        head = nn.ModuleList()
        all_out_chs = backbone_out_chs.copy()  # 合并backbone和head的输出通道记录
...
def train(hyp: Dict, opt: argparse.Namespace, device: torch.device, callbacks: Callbacks) -> Tuple[float, ...]:
    ...
    # 训练循环
    LOGGER.info(colorstr('开始训练（YOLOv8 Backbone语义分割版 - Dice损失）!'))
    t0 = time.time()
    global_step = 0
        ...
        for i, (imgs, targets, paths) in pbar:
            ...
            if RANK in {-1, 0}:
                ...
                if global_step % 10 == 0:
                    ...
                    tb_writer.add_scalar('Train/Dice_Loss', loss_items[2], global_step)  # 记录Dice损失
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='yolov8_seg.yaml', help='模型配置文件（YOLOv8语义分割版）')
    ...
    parser.add_argument('--save-dir', type=str, default='runs/train-yolov8-seg-diceloss', help='结果保存目录（Dice损失专用）')
